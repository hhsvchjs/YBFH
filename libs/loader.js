...
loader.prototype.loadImage = function (imgName, callback) {
    try {
		...
		if(name.indexOf("newEnemy") >= 0) {
			image.src = 'project/images/monsters/' + name + "?v=" + main.version;
		}
		else if(name.indexOf("prop") >= 0) {
			image.src = 'project/images/props/' + name + "?v=" + main.version;
		}
		else {
...
loader.prototype.loadAnimates = function () {
    core.animates.forEach(function (t) {
		//console.log(t); t -> hand sword zone
...
loader.prototype.loadMusic = function () {
    ...
    core.bgms.forEach(function (t) {
        ...
        if (/^.*\.mid$/i.test(t)) {
            ...
            if (core.musicStatus.audioContext!=null) {
				...
				//加载project/sounds/下的t
                core.http('GET', 'project/sounds/oggs/' + t, null, function (data) {
                    try {
                        var ff = [];
                        var mx = data.length;
                        for (var z = 0; z < mx; z++)
                            ff[z] = String.fromCharCode(data.charCodeAt(z) & 255); //返回一个字符串
                        var shouldStart = core.material.bgms[t] == 'starting'; //置为开始
    ...
    core.sounds.forEach(function (t) {
        ...
        if (core.musicStatus.audioContext != null) {
            ...
            var music = new Audio();
            music.src = 'project/sounds/oggs/' + t;
    ...
    if (core.musicStatus.startDirectly && core.bgms.length>0) {
		core.playBgm(core.bgms[0]); //bgm.mp3
	} 
}
//载入背景音乐
loader.prototype.loadOneMusic = function (name) {
    ...
    if (main.bgmRemote) music.src = main.bgmRemoteRoot+core.firstData.name + '/' + name;
		...
		music.src = 'project/sounds/bgms/' + name;
	}
    music.loop = 'loop';
...
loader.prototype.loadBgm = function (name) {
    if (!core.isset(core.material.bgms[name])) return; // 如果没开启音乐，则不预加载
    
    if (!core.musicStatus.bgmStatus) return; // 是否已经预加载过
	
    var index = core.musicStatus.cachedBgms.indexOf(name); //获取bgm缓存中name的下标
    if (index >= 0) {
        core.musicStatus.cachedBgms.splice(index, 1); //把bgm删除出数组
    }
    else {
        
        core.material.bgms[name].load(); // 预加载BGM
        // 清理尾巴
