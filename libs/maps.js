...
maps.prototype.init = function() {
    this.blocksInfo = maps_90f36752_8815_4be8_b32b_d7fad1d0542e; //道具的信息
...
maps.prototype.loadFloor = function (floorId, map) {
    ...
    map = map.map; //map.map是真map
	//地图block信息化
    var mapIntoBlocks = function(map, maps, floor, floorId) {
        var blocks = [];
        var mw = core.floors[floorId].width || 13+2;
        var mh = core.floors[floorId].height || 13+2;
        for (var i = 0; i < mh; i++) {
            for (var j = 0; j < mw; j++) {
                var block = maps.initBlock(j, i, (map[i]||[])[j]||0);
                maps.addInfo(block);
                maps.addEvent(block,j,i,floor.events[j+","+i]); //没有event的将不会添加任何事件
...
maps.prototype.initBlock = function (x, y, id) {
    ...
    id = parseInt(id);
    var tmp = {'x': x, 'y': y, 'id': id}; //某个块的x/y坐标及块id
    if (disable != null) tmp.disable = disable;
	//给地图块加buff*@*
    if (id == 17) { //id：17 -> 空气墙
        tmp.event = {"cls": "terrains", "id": "airwall", 'pd': "terrains", "noPass": true};
    }
    else if (id in this.blocksInfo) {
		tmp.event = JSON.parse(JSON.stringify(this.blocksInfo[id])); //转为json字符串
	}
    else {
        var tilesetOffset = core.icons.getTilesetOffset(id); //砖集
        if (tilesetOffset != null) {
            tmp.event = {"cls": "tileset", "id": "X" + id, 'pd': "tileset", "noPass": true};
        }
    }
    return tmp;
}
...
maps.prototype.addInfo = function (block) {
    if (core.isset(block.event)) {
		//若当前块存在敌人,则触发战斗事件
        if ((block.event.cls.indexOf("enemy") == 0 || block.event.cls.indexOf("newEnemys") == 0)
			&& !core.isset(block.event.trigger)) {
		...
		//若当前块没有定义noPass变量,且不为道具则置noPass为true
...
maps.prototype.addEvent = function (block, x, y, event) {
	...
	//若本event == null,则添加event
    if (!core.isset(block.event)) {
        block.event = {'cls': 'terrains', 'id': 'none', 'pd': "terrains", 'noPass': false};
    }
    //若event是string,则?
	...
	//若data未定义,则给与初始化
    if (!core.isset(event.data)) { //此处为false
        event.data = [];
	}
    //若disable未定义且enable已定义,则disable = enable
    if (!core.isset(block.disable) && core.isset(event.enable)) {
        block.disable = !event.enable;
    }
    //若animate为false,则animate = 1
    if (event.animate === false) { //此处undefined
...
maps.prototype.canMoveHero = function(x, y, direction, floorId) {
    ...
    //检查当前块的cannotMove
    if (core.isset(core.floors[floorId].cannotMove)) { //true
...
maps.prototype.drawBlock = function (block, animate, dx, dy) {
    ...
    var cls = block.event.cls; //各种块的放置类型，比如enemys、npc等

    var blockInfo = this.__getBlockInfo(block); //得到块的基本信息
	
    if (blockInfo == null) return; //没有块信息的
	//获取块所在的图块的位置与高度
    ...
    dy = dy || 0;
	//若block存在name,则...
    if (core.isset(block.name)) { //此处为false
...
maps.prototype.getBgFgMapArray = function (floorId, name) {
    floorId = floorId || core.status.floorId;
    if (!core.isset(floorId)) return [];
    var width = core.floors[floorId].width || 13+2;
    var height = core.floors[floorId].height || 13+2;
		...
		for (var b = 0; b < mapBlocks.length; b++) {
            ...
            if (core.isset(block.event) && !block.disable) {
                if (block.event.cls == 'autotile') {
                    ...
                    core.addGlobalAnimate(block); //添加动画
                }
            }
        }
...
maps.prototype.__getBlockInfo = function (block) {
    ...
    if (block.event.cls == 'tileset') { //此处为false
    ...
    // 不支持autotile
    else if (block.event.cls == 'autotile') { //此处为false
        return null;
    }
    // 空气墙；忽略事件
    else if (block.id == 17) { //此处为false
        if (!core.isset(core.material.images.airwall)) return null;
        image = core.material.images.airwall;
        bx = by = 0;
    }
    else {
        image = core.material.images[block.event.pd];
        bx = 0;
        by = core.material.icons[block.event.pd][block.event.id]; //与icons一一对应
