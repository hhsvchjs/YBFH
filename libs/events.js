...
events.prototype.startGame = function (hard, seed, route, callback) {
    ...
    var start = function () {
        ...
        var post_start = function () {
            ...
            core.control.triggerStatusBar('show'); //显示状态栏
			//?
...
events.prototype.doAction = function() {
    ...
    if (current.todo.length == 0) {
        if (core.calValue(current.condition)) {
...
events.prototype.getItem = function (itemId, itemNum, itemX, itemY, callback) {
    ...
    var itemCls = core.material.items[itemId].cls; //获取道具cls
    core.items.getItemEffect(itemId, itemNum); //获取道具效果
    core.removeBlock(itemX, itemY); //移除坐标处事件
    var text = '获得 ' + core.material.items[itemId].name;
    if (itemNum > 1) text += "x" + itemNum; //获取某物品*3
	//若是永久性道具,则加上效果提示
    if (itemCls === 'items') text += core.items.getItemEffectTip(itemId);
    core.drawTip(text, core.material.icons.items[itemId]); //显示提示
    core.clearMap('event', itemX * 32, itemY * 32, 32, 32); //清除地图上的某处提示
...
events.prototype.openDoor = function (id, x, y, needKey, callback) {
    ...
    //开门
	if(id.indexOf("steel") >= 0) {
		core.playSound("openDoor2.ogg");
	} else if(id.indexOf("steel") < 0) {
		core.playSound("openDoor.ogg");
	}
    var state = 0;
    ...
    for (var b = 0; b < mapBlocks.length; b++) {
        if (mapBlocks[b].x == x && mapBlocks[b].y == y && !mapBlocks[b].disable) { // 启用事件
            ...
            if (core.isset(mapBlocks[b].event) && core.isset(mapBlocks[b].event.trigger)) {
                ...
                if (trigger == 'ski') core.status.isSkiing = true; //略过事件

                //转换楼层能否穿透
                if (trigger == 'changeFloor' && !noPass) {
                    var canCross = core.flags.portalWithoutTrigger; //穿过不触发事件
                    ...
                    if (canCross) { //若可以穿过,则重新加载当前场景
                        if (core.status.replay.replaying) { //若正在回放
...
events.prototype.changeFloor = function (floorId, stair, heroLoc, time, callback, fromLoad) {
    ...
    core.clearContinueAutomaticRoute();
    core.status.replay.animate = true;
    core.dom.floorNameLabel.innerHTML = core.status.maps[floorId].title; //地图标题
	if (!core.isset(stair) && !core.isset(heroLoc))
        heroLoc = core.clone(core.status.hero.loc);
	//console.log(core.isset(stair)); false
    if (core.isset(stair)) {
...
events.prototype.checkLvUp = function () {
    var check = function () {
        ...
        //计算下一个所需要的数值
		console.log(core.status.hero.lv);
