...
utils.prototype.setLocalStorage = function(key, value) {
    try {
        var str = JSON.stringify(value); //将一个js值转化为json字符串
        var compressed = LZString.compress(str); //将json字符串压缩

        // test if we can save to localStorage
        localStorage.setItem("__tmp__", compressed); //设置__tmp__为compressed
		//如果解压的文件与str相等,则设置
        ...
        localStorage.removeItem("__tmp__"); //移除__tmp__
        return true;
    }
    catch (e) {
        console.log(e);
