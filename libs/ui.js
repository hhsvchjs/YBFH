...
ui.prototype.getTitleAndIcon = function (content) {
    ...
    var getInfo = function (v) {
        ["enemy48", "enemys", "npc48", "npcs", 'newEnemy1'].forEach(function (x) {
...
//             if (content.indexOf("\\t[") == 0) str = content.substring(3, index);
			...
			if(content.indexOf("\t[") == 0) {
				str = content.substring(2, index);
			} else if(content.indexOf("\\t[") == 0) {
				str = content.substring(3, index);
			}
...
ui.prototype.drawTextBox = function(content, showAll) {
    ...
    // 获得name, image, icon
    var info = this.getTitleAndIcon(content); //获取animate、content、icon、id、image、name等
    ...
    var isWindowSkin = false; //区分对话框与提示框
    if (typeof background == 'string') { //此处为false
    ...
    var left = 7, right = 416 + 32 * 2 - left, width = right - left, validWidth = width - leftSpace - rightSpace;
    // 对话框效果：改为动态计算
    if (core.isset(px) && core.isset(py)) {
        ...
        left = core.clamp(32 * px + 16 - width / 2 - core.bigmap.offsetX, 7, 416 + 32 * 2 - 7 - width);
        right = left + width;
    }

    var content_left = left + leftSpace;
    var height = 30 + (textfont + 5) * core.splitLines("ui", realContent, validWidth, font).length; //字符串换行分割
    ...
    if (position == 'center') { //标题居中
        top = parseInt((416 + 32 * 2 - height) / 2);
    }
    else if (position == 'up') {
        if (px == null || py == null)
            top = 5 + offset;
        ...
        if (px == null || py == null)
            top = 416 + 32*2 - height - 5 - offset;
        else {
            top = 32 * py + 32 + yoffset;
            top -= core.bigmap.offsetY;
        }
    }
    var bottom = top + height; //框底

    if (isWindowSkin) {
        core.setAlpha('ui', alpha); //设定某个canvas的alpha值
...
ui.prototype.drawScrollText = function (content, time, callback) {
    ...
    // 计算总高度，按1.4倍行距计算
    var width = 416+32*2, height = textfont * 1.4 * lines;
...
ui.prototype.drawChoices = function(content, choices) {
    ...
    var left=parseInt((416+32*2 - width) / 2); // 左边界
    // 高度
...
ui.prototype.drawConfirmBox = function (text, yesCallback, noCallback) {
    ...
    var right = 416+32*2 - left, bottom = 416+32*2 - 140, width = right - left, height = bottom - top;
...
ui.prototype.drawBattleAnimate = function(monsterId, callback) {
    ...
    core.clearMap('ui');
    var left=10, right=416+32*2-2*left;
    ...
    var top = (416+32*2-height)/2, bottom = height;
...
ui.prototype.drawWaiting = function(text) {
    ...
    var left = 208-parseInt(right/2), top = 208 - 32 - 16, bottom = 416+32*2 - 2 * top;
...
ui.prototype.drawPagination = function (page, totalPage, top) {
    ...
    core.setTextAlign('ui', 'left');
    core.fillText('ui', page + " / " + totalPage, parseInt((416+32*2 - length) / 2), top*32+19);
...
ui.prototype.drawBook = function (index) {
    ...
    core.fillRect('ui', 0, 0, 416+32*2, 416+32*2);
    ...
    for (var i = 0; i < enemys.length; i++) {
        ...
        if (core.isset(core.material.icons.enemy48[enemy.id]))
            pd = 'enemy48';
		if(core.isset(core.material.icons.newEnemy1[enemy.id])) {
			pd = 'newEnemy1';
		}
		if(core.isset(core.material.icons.newEnemy2[enemy.id])) {
			pd = 'newEnemy2';
		}
		if(core.isset(core.material.icons.newEnemy3[enemy.id])) {
			pd = 'newEnemy3';
		}
		if(core.isset(core.material.icons.newEnemy4[enemy.id])) {
			pd = 'newEnemy4';
		}
        var height = pd == 'enemy48' ? 48 : 32;
        var animate = pd == 'enemy48' ? 4 : 2;

        // 怪物
        core.status.boxAnimateObjs.push({
            'bgx': 22, 'bgy': 62 * i + 22, 'bgWidth': 42, 'bgHeight': 42,
            'x': 27, 'y': 62 * i + 27, 'height': 32, 'animate': animate,
            'image': core.material.images[pd],
            'pos': core.material.icons[pd][enemy.id] * height
        });
        ...
        if (index == start+i) {
            core.strokeRect('ui', 10, 62 * i + 13, 416+32*2-10*2,  62, '#FFD700');
        }
...
ui.prototype.drawBookDetail = function (index) {
    ...
    var height = 416+32*2 - 10 - Math.min(416+32*2-24*(contents.length+1)-65, 250);
    var top = (416+32*2-height)/2, bottom = height;
...
ui.prototype.drawFly = function(page) {
    ...
    core.clearMap('ui');
    core.setAlpha('ui', 0.85);
    core.fillRect('ui', 0, 0, 416+32*2, 416+32*2, '#000000');
    core.setAlpha('ui', 1);
    core.setTextAlign('ui', 'center');
    var globalFont = core.status.globalAttribute.font
...
ui.prototype.drawMaps = function (index, x, y) {
    ...
    if (!core.isset(index)) {
        ...
        core.fillRect('ui', 0, 0, 416+32*2, 416+32*2, 'rgba(0,0,0,0.4)');

        core.strokeRect('ui', 66, 2, 284, 60, "#FFD700", 4);
        core.strokeRect('ui', 2, 66, 60, 284);
        core.strokeRect('ui', 66, 416-62+32*2, 284, 60);
        core.strokeRect('ui', 416-62+32*2, 66, 60, 284);
        ...
        core.strokeRect('ui', 2, 416-30+32*2, 28, 28);
        core.fillText('ui', 'M', 16, 408);
        core.strokeRect('ui', 416-30+32*2, 2, 28, 28);
        ...
        core.fillText('ui', "右", 384+32*2, top);
        core.fillText('ui', "移", 384+32*2, top+32);
        core.fillText('ui', "地", 384+32*2, top+32*2);
        core.fillText('ui', "图", 384+32*2, top+32*3);
        core.fillText('ui', "[D]", 384+32*2, top+32*4);
    ...
    var floorId = core.floorIds[index], mw = core.floors[floorId].width||13+2, mh = core.floors[floorId].height||13+2;
    ...
    core.clearLastEvent();
    this.drawThumbnail(floorId, 'ui', core.status.maps[floorId].blocks, 0, 0, 416+32*2, x, y);

    // 绘图
    if (core.status.event.data.paint) {
        var offsetX = core.clamp(x-6, 0, mw-13-2), offsetY = core.clamp(y-6, 0, mh-13-2);
        ...
        core.drawImage('ui', core.bigmap.tempCanvas.canvas, offsetX*32, offsetY*32, 416+32*2, 416+32*2, 0, 0, 416+32*2, 416+32*2);
    }
    ...
    if (!all && (mw>13+2 || mh>13+2)) text+=" ["+(x-6)+","+(y-6)+"]";
