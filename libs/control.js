...
control.prototype.drawHero = function (direction, x, y, status, offset) {
    ...
    core.bigmap.offsetX = core.clamp((x - 6) * 32 + offsetX, 0, 32*core.bigmap.width-416-32*2);
    core.bigmap.offsetY = core.clamp((y - 6) * 32 + offsetY, 0, 32*core.bigmap.height-416-32*2);
...
control.prototype.setWeather = function (type, level) {
    ...
    core.createCanvas('weather', 0, 0, 416 + 32 * 2, 416 + 32 * 2, 80);
...
control.prototype.updateDamage = function (floorId, canvas) {
    ...
    if (core.flags.displayEnemyDamage || core.flags.displayCritical) {
        ...
        for (var b = 0; b < mapBlocks.length; b++) {
            ...
            if (core.isset(mapBlocks[b].event) && (mapBlocks[b].event.cls.indexOf('enemy') == 0 || mapBlocks[b].event.cls.indexOf('newEnemy') == 0)
                && !mapBlocks[b].disable) {
                // 判定是否显伤
    ...
    if (core.flags.displayExtraDamage && core.isset((core.status.checkBlock||{}).damage)) {
        ...
        if (floorId != core.status.floorId) {
            ...
            core.bigmap.width = core.floors[floorId].width || 13+2;
            core.bigmap.height = core.floors[floorId].height || 13+2;
...
control.prototype.playBgm = function (bgm) {
    if (main.mode != 'play') return; //如果模式不为播放
	
    if (!core.musicStatus.bgmStatus) return; // 如果不允许播放
    
    if (!core.isset(core.material.bgms[bgm])) return; // 音频不存在
...
//             core.statusBar.image[t].style.display = 'none'; //none
//         });
		["book","save","load"].forEach(function (t) {
			core.statusBar.image[t].style.display = 'none'; //none
		});
        ...
        ["fly", "toolbox", "shop", "settings"].forEach(function (t) {
            core.statusBar.image[t].style.display = 'none'; //block
        });
// 		["book","save","load"].forEach(function (t) {
// 			core.statusBar.image[t].style.display = 'block'; //block
// 		});
...
control.prototype.resize = function(clientWidth, clientHeight) {
    ...
    var DEFAULT_CANVAS_WIDTH = 422+32*2; //默认地图宽
    var DEFAULT_BAR_WIDTH = 132+30; //默认边框宽
    var BASE_LINEHEIGHT = 32; //默认行高
    var SPACE = 3; //默认边界宽
    var DEFAULT_FONT_SIZE = 16; //默认字体大小
    var ADAPT_WIDTH = DEFAULT_CANVAS_WIDTH; 
    var CHANGE_WIDTH = DEFAULT_CANVAS_WIDTH + DEFAULT_BAR_WIDTH; //默认界面宽
    
    var width = clientWidth; //客户端屏宽
    var isHorizontal = false; //默认竖屏
	//如果宽 > 高 && 高 < 默认宽则横屏
    ...
    var count = toDraw.length; //需要被绘制的状态栏项目数
	//调整状态栏字体大小
    ...
    var statusLineHeight = BASE_LINEHEIGHT * 12 / count; //状态栏行高
    var statusLineFontSize = DEFAULT_FONT_SIZE; //状态栏字体
    if (count > 12) statusLineFontSize = statusLineFontSize * 12 / count;

    var shopDisplay;
	
    var borderColor = (core.status.globalAttribute || core.initStatus.globalAttribute).borderColor;

    statusBarBorder = '3px ' + borderColor + ' solid'; //给状态栏画个边界框
    toolBarBorder = '3px ' + borderColor + ' solid'; //给工具栏画个边界框
    ...
    if (width < CHANGE_WIDTH) {
        ...
        // statusBarHeight = statusLineHeight * count + SPACE * 2; //一共有12行
        statusBackground = (core.status.globalAttribute || core.initStatus.globalAttribute).statusLeftBackground;
        statusBarHeight = gameGroupHeight - SPACE;
        statusCanvasWidth = toolBarWidth + SPACE;
        statusCanvasHeight = statusBarHeight;
        ...
        toolBarTop = statusLineHeight * (count) + SPACE * 2 + 9; //+9
        toolBarHeight = DEFAULT_CANVAS_WIDTH - toolBarTop; //toolBarHeight
		toolsBackground = 'transparent';
	...
	//状态栏canvas化
    if (core.domStyle.isVertical) {
        core.dom.statusCanvas.width = 416+32*2;
        core.dom.statusCanvas.height = col * BASE_LINEHEIGHT + SPACE + 6;
    }
    else {
        core.dom.statusCanvas.width = 129+20;
        core.dom.statusCanvas.height = 416+32*2;
    }
    if (core.isPlaying())
        core.updateStatusBar();
}
