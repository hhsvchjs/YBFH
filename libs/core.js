...
core.prototype.init = function (coreData, callback) {
	...
	//游戏基本信息
    core.dom.versionLabel.innerHTML = core.firstData.version; //版本号
    core.dom.logoLabel.innerHTML = core.firstData.title; //logo名
    document.title = core.firstData.title + " - HTML复刻版"; //网站名
    document.getElementById("startLogo").innerHTML = core.firstData.title; //主界面logo名
    //道具、敌人的加载
    ...
    if (main.mode == 'play') {
        for (var enemyId in core.material.enemys) {
			//console.log(enemyId); id即name
            core.material.enemys[enemyId].id = enemyId;
        }
    }
	
    core.material.icons = core.icons.getIcons(); //获取icon
    core.material.events = core.events.getEvents(); //获取event
	//一些联网处理
	...
	//背景音乐的加载
    if (core.platform.isPC) { //如果是PC端直接加载
 ...
 * 播放背景音乐-管理处
 */
core.prototype.playBgm = function (bgm) { //开始播放bgm bgm.mp3
    core.control.playBgm(bgm);
}

core.prototype.pauseBgm = function () { //暂停播放bgm
    core.control.pauseBgm();
}

core.prototype.resumeBgm = function () { //恢复播放bgm
    core.control.resumeBgm();
}

core.prototype.loadBgm = function (bgm) { //预加载bgm
    core.loader.loadBgm(bgm);
}

core.prototype.freeBgm = function (bgm) { //释放bgm缓存
    core.loader.freeBgm(bgm);
}

core.prototype.playSound = function (sound) { //播放音频
