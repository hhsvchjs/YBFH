var functions_d6ad677b_427a_4623_b50f_a445a3b0ef8a = {
	"events": {
		//游戏开始前的一些初始化操作
		"initGame": function() {
			//若是bigKeyIsBox,则重新给bigKey赋值
			if (core.flags.bigKeyIsBox) { //此处为false
				core.material.items.bigKey = {
					'cls': 'items',
					'name': '钥匙盒'
				};
			}
			//若是四周可破坏的破墙镐,则...
			if (core.flags.pickaxeFourDirections) { //此处为false
				core.material.items.pickaxe.text = "可以破坏勇士四周的墙";
			}
			//若是四周可炸掉的炸弹,则...
			if (core.flags.bombFourDirections) { //此处为false
				core.material.items.bomb.text = "可以炸掉勇士四周的怪物";
			}
			//若是四周可融掉的炸弹.则...
			if (core.flags.snowFourDirections) { //此处为false
				core.material.items.bomb.text = "可以将四周的熔岩变成平地";
			}
			//若启用装备栏按钮,则修改状态栏的fly为equipbox
			if (core.flags.equipboxButton) { //此处为false
				core.statusBar.image.fly.src = core.statusBar.icons.equipbox.src;
				core.flags.equipment = true;
			}
			//若启用装备栏,则修改cls为equips
			if (core.flags.equipment) { //此处为false
		...
		"setInitData": function(hard) {
			...
			if (hard == 'Easy') {
				core.setFlag('hard', 1);
				//初心福利 黄钥匙+5 护甲+3 初始攻击+1 初始防御+1 初始经验+40
				core.setItem("yellowKey", 5);
				core.setStatus("atk", 4);
				core.setStatus("def", 3);
				core.setStatus("experience", 40);
			}
			//普通难度
			if (hard == 'Normal') {
				core.setFlag('hard', 2);
		...
		"changingFloor": function(floorId, heroLoc, fromLoad) {
			...
			if (core.isset(color)) {
				core.fillRect('curtain', 0, 0, 416 + 32 * 2, 416 + 32 * 2, core.arrayToRGBA(color));
		...
		"afterChangeFloor": function(floorId, fromLoad) {
			...
			var visited = core.getFlag("__visited__", []); //simple0
		...
		"getSpecials": function() {
			...
			return [
				...
				[11, "吸血", function(enemy) {
					return "战斗前，怪物首先吸取角色的" + Math.floor(100 * enemy.value || 0) + "%生命（约" + Math.floor((enemy.value || 0) * core.getStatus(
						'hp')) + "点）作为伤害" + (enemy.add ? "，并把伤害数值加到自身生命上" : "");
				}],
				[12, "中毒", "战斗后，勇士陷入中毒状态，每一步损失生命" + core.values.poisonDamage + "点"],
				[13, "衰弱", "战斗后，勇士陷入衰弱状态，攻防暂时下降" + (core.values.weakValue >= 1 ? core.values.weakValue + "点" : parseInt(core.values
					.weakValue * 100) + "%")],
				[14, "诅咒", "战斗后，勇士陷入诅咒状态，战斗无法获得金币和经验"],
				[15, "领域", function(enemy) {
					return "经过怪物周围" + (enemy.range || 1) + "格时自动减生命" + (enemy.value || 0) + "点";
				}],
				[16, "夹击", "经过两只相同的怪物中间，勇士生命值变成一半"],
				[17, "仇恨", "战斗前，怪物附加之前积累的仇恨值作为伤害" + (core.flags.hatredDecrease ? "；战斗后，释放一半的仇恨值" : "") + "。（每杀死一个怪物获得" + (core.values
					.hatred || 0) + "点仇恨值）"],
				...
				[24, "激光", function(enemy) {
					return "经过怪物同行或同列时自动减生命" + (enemy.value || 0) + "点";
				}],
				[25, "光环", function(enemy) {
					return "同楼层所有怪物生命提升" + (enemy.value || 0) + "%，攻击提升" + (enemy.atkValue || 0) + "%，防御提升" + (enemy.defValue || 0) +
						"%，" + (enemy.add ? "可叠加" : "不可叠加");
				}]
			];
		},
		...
		"onKeyUp": function(keyCode, altKey) {
			...
			switch (keyCode) {
				...
				case 52: // 快捷键4：破冰/冰冻/地震/上下楼器/... 其他道具依次判断
					{
						var list = ["icePickaxe", "snow", "earthquake", "upFly", "downFly", "jumpShoes", "lifeWand", "poisonWine",
							"weakWine", "curseWine", "superWine"
						];
	...
	"control": {
		...
		"updateStatusBar": function() {
			...
			core.events.checkLvUp(); //检查等级
			if (core.flags.enableHPMax) { //检查HP上限
				core.setStatus('hp', Math.min(core.getStatus('hpmax'), core.getStatus('hp')));
			}
			core.events.setFloorName(); //设置楼层名
			core.statusBar.name.innerHTML = core.getStatus('name'); //设置勇士名字和图标
			var lvName = core.getLvName();
			core.statusBar.lv.innerHTML = lvName; //设置等级名称
			...
			statusList.forEach(function(item) {
				if (core.isset(core.status.hero[item])) //数值向下取整
					core.status.hero[item] = Math.floor(core.status.hero[item]);
				//装备效果加成
				var value = Math.floor(core.getStatus(item) * core.getFlag('equip_' + item + '_buff', 1));
				core.statusBar[item].innerHTML = core.formatBigNumber(value); //大数据格式化
			});
						...
						if (core.enemys.hasSpecial(enemy.special, 18) && !core.hasFlag("no_snipe")) {
							for (var dx = -1; dx <= 1; dx++) {
								for (var dy = -1; dy <= 1; dy++) {
									...
									var nx = x + dx,
										ny = y + dy;
									if (nx < 0 || nx >= core.bigmap.width || ny < 0 || ny >= core.bigmap.height || Math.abs(dx) + Math.abs(dy) >
										1) continue;
									core.status.checkBlock.damage[nx + ny * core.bigmap.width] += enemy.value || 0;
								}
							}
				...
				for (var x = 0; x < core.bigmap.width; x++) {
					for (var y = 0; y < core.bigmap.height; y++) {
						...
						if (has) {
							...
							if (leftHp > 1)
								core.status.checkBlock.damage[x + core.bigmap.width * y] += Math.floor((leftHp + (core.flags.betweenAttackCeil ?
									0 : 1)) / 2);
						}
					}
				}
	...
	"ui": {
		"drawStatusBar": function() {
			...
			ctx.font = 'italic bold 18px Consolas';
			...
			var toDraw = ["floor", "lv", "hp", "mana", "atk", "def", "mdef", "money"];
			...
			this.drawLight = function(color, lights, lightDec) {
				...
				lights.forEach(function(light) {
					...
					var x = light[0],
						y = light[1],
						r = light[2],
						o = 255 * (1 - core.clamp(light[3], 0, 1));
					// 计算衰减距离
					var decDistance = parseInt(r * lightDec),
						leftDistance = r - decDistance;
					// 正方形区域的直径和左上角坐标
					var d = r * 2,
						sx = x - r,
						sy = y - r;
